...
namespace sluggish.imageprocessing
{
	...
	public class region
	{
	    ...
	    public String classification; // eg text, datamatrix
                      ...
                      int downsampling_factor)
		{
            ...
            for (int y = ty; y < ty + height; y++)
            {
		        for (int x = tx; x < tx + width; x++)
		        {
		            ...
		            {   
		                ...
		                {
		                    shape[xx, yy] = region_image[x/downsampling_factor, 
		                                                 y/downsampling_factor];
		            		            
                            int n1 = (yy * width) + xx;
                            int n2 = (y * bmp_width) + x;
		    ...
		    if (aspect_ratio > 1.3f)
		        ...
		        // find corners
		        fitCorners(corner_features, vertex_inflation, vertex_angular_offset);
                                     ...
                                     int corner_samples_percent)
        {
            ...
            int background_downsampling_factor = 4;
                                ...
                                if ((xx > -1) && (xx < width / background_downsampling_factor))
                                {
                                    ...
                                    for (int yy = small_y - radius; yy <= small_y + radius; yy++)
                                    {
                                        if ((yy > -1) && (yy < height / background_downsampling_factor))
                                        {
                                            ...
                                            int probability = radiusSquared - distSquared;  // this should probably have gaussian distribution, but here we just use something linear for speed                                            
        ...
        /// <param name="remove_solitary_pixels">remove any pixels on ther own as a way of reducing noise</param>
        public void CreateBinaryImage(float binary_image_threshold, bool remove_solitary_pixels)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    int intensity = (int)mono_image[(y * width) + x];
            ...
            if (remove_solitary_pixels)
            {
                // remove solitary pixels            
                binary_image = sluggish.utilities.image.removeSolitaryPixels(binary_image);
            }
        }
                                           ...
                                           ref ArrayList cell_perimeter)
        {
            ...
            int left = (int)Math.Round(perimeter.left());
            int top = (int)Math.Round(perimeter.top());
            int right = (int)Math.Round(perimeter.right());
            int bottom = (int)Math.Round(perimeter.bottom());
                                        ...
                                        bool equalise_spacings)
        {
            ...
            int radius = (int)Math.Round(spot_radius * 1.5f / pixels_per_index);
            for (int i = 0; i < grid_spacing_maxima.Length-1; i++)
            {
                if (grid_spacing_maxima[i] > 0)
                {
	                int max_index = i + radius;
                                 ...
                                 ref float shear_angle_radians)
        {
            ...
            horizontal_maxima = fitGridMaxima(grid_spacing_horizontal, pixels_per_index, min_grid_threshold, true, 2, true); 
            vertical_maxima = fitGridMaxima(grid_spacing_vertical, pixels_per_index, min_grid_threshold, true, 2, true);
        ...
        */
                                ...
                                float max_radius_variance)
        {
            ...
            if (spot_radius == 0)
                spot_radius = getSpotAverageRadius(black_on_white);

            // spot radius as an integer
            int spot_radius_rounded = (int)Math.Round(spot_radius);
                        
            //if (spot_radius_rounded < 1) spot_radius_rounded = 1;
        ...
        public float getSpotAverageRadius(bool black_on_white)
        {
            ...
            int min_diameter = height / 35;
            if (min_diameter < 1) min_diameter = 1;
            
            Console.WriteLine(polygon.getShortestSide().ToString());
