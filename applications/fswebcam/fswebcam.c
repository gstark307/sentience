...
char *fswc_strftime(char *dst, size_t max, char *src,
                    time_t timestamp, int gmt, int device_index)
{
	...
	*dst = '\0';
	...
	if (device_index > -1)
	{
	    ...
	    {
	        int i = 0;
	        for (i = len; i >= pos; i--) dst[i] = dst[i-1];
	        dst[pos] = device_index + 48;
	    }
	}
...
int fswc_output(fswebcam_config_t *config, char *name, gdImage *image, int device_index)
{
	...
	fswc_strftime(filename, FILENAME_MAX, name,
	              config->start, config->gmt, device_index);
...
void fswc_get_devices(fswebcam_config_t *config)
{
    config->no_of_devices = 1;
    int index, prev_i, i, j, len, dlen=0;
    i = 0;
    while ((config->device[i] != 0) && (config->device[i] != 13) && (config->device[i] != 10))
    {
        if (config->device[i] == ',')
            config->no_of_devices++;
        i++;
        dlen++;
    }
    
            
    if (config->no_of_devices > 1)
    {
        /* multiple video devices were specified */
        index = 0;
        prev_i = 0;
        for (i = 0; i < dlen; i++)
        {
            ...
            {
                len = i - prev_i;
                config->devices[index] = malloc(len);
                for (j = prev_i; j < i; j++)
                    config->devices[index][j - prev_i] = config->device[j];
                prev_i = i+1;
                index++;
            }
        }
    ...
    {
        /* only a single device was specified */
        config->no_of_devices = 1;
        config->devices[0] = config->device;
    }
}
...
int fswc_grab(fswebcam_config_t *config)
{
	uint32_t frame;
	uint32_t x, y;
	avgbmp_t *abitmap[MAX_DEVICES], *pbitmap;
	gdImage *image, *original;
	uint8_t modified;
	src_t src[MAX_DEVICES];
	
	/* Record the start time. */
	config->start = time(NULL);
		
	fswc_get_devices(config);

        if (config->no_of_devices > 1)
                HEAD("--- Capturing from %d devices...", config->no_of_devices);

        int dev = 0;
        for (dev = 0; dev < config->no_of_devices; dev++)
        {
        	/* Set source options... */
        	memset(&src[dev], 0, sizeof(src[dev]));
        	src[dev].input      = config->input;
        	src[dev].tuner      = config->tuner;
        	src[dev].frequency  = config->frequency;
        	src[dev].delay      = config->delay;
        	src[dev].timeout    = 10; /* seconds */
        	src[dev].use_read   = config->use_read;
        	src[dev].list       = config->list;
        	src[dev].palette    = config->palette;
        	src[dev].width      = config->width;
        	src[dev].height     = config->height;
        	src[dev].option     = config->option;
               	
        	HEAD("--- Opening %s...", config->devices[dev]);
        	
        	if(src_open(&src[dev], config->devices[dev]) == -1) return(-1);
        	
        	/* The source may have adjusted the width and height we passed
        	 * to it. Update the main config to match. */
        	config->width  = src[dev].width;
        	config->height = src[dev].height;
        	
        	/* Allocate memory for the average bitmap buffer. */
        	abitmap[dev] = calloc(config->width * config->height * 3, sizeof(avgbmp_t));
        	if(!abitmap[dev])
        	{
        		ERROR("Out of memory.");
        		return(-1);
        ...
        if(config->frames == 1)
        {
                if (config->no_of_devices == 1)
                        ...
                        HEAD("--- Capturing frames from %i devices...", config->no_of_devices);
        }
        else
        {
                if (config->no_of_devices == 1)
                        HEAD("--- Capturing %i frames...", config->frames);
                else
                        HEAD("--- Capturing %i frames from &i devices...", config->frames, config->no_of_devices);
        }
        
        unsigned char* img_data[MAX_DEVICES][config->frames];
        int img_data_length[MAX_DEVICES][config->frames];
        int i;
       	...
       	for(frame = 0; frame < config->frames; frame++)
       	{
                for (dev = 0; dev < config->no_of_devices; dev++)
                {        	
        		if(src_grab(&src[dev]) == -1) break;
        		
        		/* store the grabbed image data */
        		img_data[dev][frame] = (unsigned char *)malloc(src[dev].length);
       		        memcpy(img_data[dev][frame], src[dev].img, src[dev].length);
       		        img_data_length[dev][frame] = src[dev].length;
                }
        }
        ...
        /* create average images */
       	for(frame = 0; frame < config->frames; frame++)
       	{
        		...
        		if(!frame && config->dumpframe)
        		{
        			...
        			{
        				size_t l;
        				l = fwrite(src[dev].img, 1, src[dev].length, f);
        				fclose(f);
        			}
        		}
        		
        		/* copy stored data back into src */
        		src[dev].length = img_data_length[dev][frame];
        		memcpy(src[dev].img, img_data[dev][frame], src[dev].length);
        		
        		/* Add frame to the average bitmap. */
        		switch(src[dev].palette)
        		{
        		case SRC_PAL_PNG:
        			fswc_add_image_png(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_JPEG:
        		case SRC_PAL_MJPEG:
        			fswc_add_image_jpeg(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_RGB32:
        			fswc_add_image_rgb32(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_BGR32:
        			fswc_add_image_bgr32(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_RGB24:
        			fswc_add_image_rgb24(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_BGR24:
        			fswc_add_image_bgr24(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_BAYER:
        			fswc_add_image_bayer(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_YUYV:
        		case SRC_PAL_UYVY:
        			fswc_add_image_yuyv(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_YUV420P:
        			fswc_add_image_yuv420p(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_NV12MB:
        			fswc_add_image_nv12mb(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_RGB565:
        			fswc_add_image_rgb565(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_RGB555:
        			fswc_add_image_rgb555(&src[dev], abitmap[dev]);
        			break;
        		case SRC_PAL_GREY:
        			fswc_add_image_grey(&src[dev], abitmap[dev]);
        			break;
        		}
	...
	if (config->no_of_devices == 1)
  	        ...
  	        HEAD("--- Processing captured images...");
	...
	{	
        	...
        	for(x = 0; x < config->jobs; x++)
        	{
        		...
        		case OPT_SAVE:
        		        if (config->no_of_devices == 1)
                			fswc_output(config, options, image, -1);
                		else
                			fswc_output(config, options, image, dev);                		
        			modified = 0;
        			break;
        		case OPT_EXEC:
...
int fswc_free_config(fswebcam_config_t *config)
{
        ...
        {
	        int i = 0;
        	for (i = 0; i < config->no_of_devices; i++)
