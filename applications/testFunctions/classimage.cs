...
namespace WindowsApplication1
{
    public class classimage
    {
        ...
        public void createImage(int wdth, int hght)
        {
            ...
            Integral = new long [width,height,4];
        ...
        public void updateIntegralImage()
        {
            ...
            Texture = (int)(textureVal / (width * height));
        }
        ...
        //------------------------------------------------------------------------------------------------------------------------
        public long getIntegral(int tx, int ty, int bx, int by, int index)
        {
            if ((tx > 0) && (ty > 0) && (bx < width) && (by < height))
        ...
        //------------------------------------------------------------------------------------------------------------------------
        public long detectFeature(int x, int y, int wdth, int hght, int featureType)
        {
            ...
            switch(featureType)
            {
                ...
                case 2:  //C
	            {
	                ...
	                v = Math.Abs((area[1] * 2) - (area[0] + area[2]));
	                break;
	            }
                ...
                case 3:  //D
	            {
	                ...
	                v = Math.Abs((area[1] + area[2]) - (area[0] + area[3]));
	                break;
	            }
        ...
        //------------------------------------------------------------------------------------------------------------------------
        public int createFeatureList(int tx, int ty, int bx, int by, ref int threshold, int resolution_x, int resolution_y, long[,] result, ref int cx, ref int cy, bool clearTopFeatures)
        {
            float x,y,xinc,yinc;
            int i,j,winner,hits,xx,yy;
            long v,vmax,p,pmax,pixels;
            long[] tmp = new long[6];
            bool found;
            ...
            if (initFeatureList==0)
            {
	            ...
	            topFeature = new long [OptimalNoOfFeatures,6];
                initFeatureList=1;
            }
        ...
        public void updateFromBitmapInvert(Byte[] bmp, int RGBformat, int wdth, int hght)
        {
            int x, y, xx, yy;
            Byte r, g, b;
            int p,max;
        ...
        //draws a green box
        //------------------------------------------------------------------------------------------------------------------------
        public void DrawBox(int tx, int ty, int bx, int by, int r, int g, int b, int lineWidth)
        {
            drawLine(tx, ty, bx, ty, r, g, b, lineWidth);
            drawLine(bx, ty, bx, by, r, g, b, lineWidth);
            drawLine(tx, by, bx, by, r, g, b, lineWidth);
            drawLine(tx, ty, tx, by, r, g, b, lineWidth);
        }
        ...
        public void getOrientations(classimage sourceimage, classimage edgesimage, int radius, int no_of_angles, int tx, int ty, int bx, int by, int[,,] radius_lookup, int trim, ref int average_confidence, classimage secondaryimage)
        {
            ...
            confidence_thresh = average_confidence*15/100;
            local_radius=1;
       ...
       public void detectEdgesStandard(classimage sourceimage, int minThresh, int patchSize)
       {
           ...
           for (y=halfPatch;y<height-halfPatch;y++)
           {
               for (x=halfPatch;x<width-halfPatch;x++)
	           {
	               ...
	               p2 = (int)sourceimage.getIntegral(x-halfPatch,y-halfPatch,x+halfPatch,y+halfPatch,0);
       ...
       public void drawLine(int x1, int y1, int x2, int y2, int r, int g, int b, int linewidth)
       {
        ...
        public float Gaussian(float fraction)
        {
            float prob = ((float)Math.Cos(fraction * Math.PI) + 1.0f) / 2;
            return (prob);
        }

        public void addGaussianLine(int x1, int y1, int x2, int y2, int linewidth, int additive)
        {
            int w, h, x, y, step_x, step_y, dx, dy, xx2, yy2;
            int cx, cy;
                        ...
                        for (xx2 = x - linewidth; xx2 <= x + linewidth; xx2++)
                            for (yy2 = y - linewidth; yy2 <= y + linewidth; yy2++)
                            {
                                if ((xx2 >= 0) && (xx2 < width) && (yy2 >= 0) && (yy2 < height))
                                {
                                    ...
                                    if (image[xx2, yy2, 0] < 255 - (int)incr)
                                        image[xx2, yy2, 0] += (Byte)incr;
                                    //image[xx2, yy2, 0] = (Byte)255;
                                }
                            }
