...
namespace surveyor.vision
{
    ...
    /// </summary>
    public sealed class StereoVisionContoursImage
    {
        ...
        /// constructor
        /// </summary>
        public StereoVisionContoursImage()
        {
            init();
        }
        ...
        private static void updateIntegralImage(byte[] bmp,
        ...
        {
            updateIntegralImage(image, width, height, Integral);
		}
        ...
        private static int getIntegral(int[] Integral, int tx, int ty, int bx, int by, int image_width)
        {
		...
		public void updateAverages()
		{
            ...
            for (int y = 1; y < height; y++)
                row_average[y] = getIntegral(Integral, 0, y-1, width - 1, y, width) / width;

            // calculate the average intensity for each column
            for (int x = 1; x < width; x++)
                column_average[x] = getIntegral(Integral, x-1, 0, x, height - 1, width) / height;
			...
			for (int x = 1; x < width-1; x++)
			{
				...
				{
					above = getIntegral(Integral, x, y - edge_radius, x+1, y, width); 
					below = getIntegral(Integral, x, y, x+1, y + edge_radius, width);
        ...
        private float detectBlob_centreSurround(int x, int y, int blobradius_x,
                                                ref float diff2, 
                                                ref float diff_row, 
                                                ref float diff_column)
        {
            // get the total pixel intensity for the surround region
            outer = getIntegral(Integral, tx1, ty1, bx1, by1, width);
            
            // get the total pixel intensity for the centre region
            inner = getIntegral(Integral, tx2, ty2, bx2, by2, width);
            ...
            leftside = getIntegral(Integral, tx1, ty1, tx1 + blobradius_x, by1, width);
                ...
                for (x = tx; x <= bx; x += step_size)
                {
                    ...
                    if (abs_centre_surround_diff > abs_left_right_diff)
                    {
                            ...
                            (abs_centre_surround_diff > abs_best_response))
                        {
                            ...
                            pattern[y][i] = StereoVisionContours.PATTERN_CENTRE_SURROUND;
                        }
                    }
                    else
                    {
                            ...
                            (abs_left_right_diff > abs_best_response))
                        {
                            ...
                            points[y][i][2] = column_diff;
                            scales[y][i] = (byte)scale;
                            pattern[y][i] = StereoVisionContours.PATTERN_LEFT_RIGHT;
                        }
                    }
                    i++;
