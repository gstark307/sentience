...
namespace SceneLibrary
{
    ...
    public class SceneLib
    {
                          ...
                          ref float sd0ptr, ref float sd1ptr)
        {
            ...
            for (int y0copy = y0lim - 1; y0copy >= 0; y0copy--)
            {
                ...
                for (int x0copy = x0lim - 1; x0copy >= 0; x0copy--)
                {
                    ...
                    v0 = p0[(yy0 * patchwidth) + x0 + x0copy];
                    v1 = p1[(yy1 * patchwidth) + x1 + x0copy];
		...
		/// <returns></returns>
        public static void updateIntegralImage(byte[] image, 
		                                       ...
		                                       ref int[][] Integral)
        {
			...
			if (Integral == null)
			{
				...
				for (x = 0; x < image_width; x++)
					Integral[x] = new int[image_height];
			}
        ...
        public static int getIntegral(int tx, int ty, int bx, int by, int[][] Integral)
        {
                          ...
                          uint ufinish, uint vfinish)
        {
            ...
            for (int x = (int)ustart; x < ufinish; x++)
            {
                for (int y = (int)vstart; y < vfinish; y++)
                {
                    ...
                    bx = (int)(tx + BOXSIZE);
                    by = (int)(ty + BOXSIZE);
                    long leftval = getIntegral(tx, ty, x, by, Integral);
                    long rightval = getIntegral(x, ty, bx, by, Integral);
                    long topval = getIntegral(tx, ty, bx, y, Integral);
                    long bottomval = getIntegral(tx, y, bx, by, Integral);
                          ...
                          uint ufinish, uint vfinish)
        {
            ...
            if (ufinish > image_width - (BOXSIZE - 1) / 2 - 1)
                ufinish = (uint)(image_width - (BOXSIZE - 1) / 2 - 1);
            if (vstart < (BOXSIZE - 1) / 2 + 1)
                vstart = (BOXSIZE - 1) / 2 + 1;
            if (vfinish > image_height - (BOXSIZE - 1) / 2 - 1)
                vfinish = (uint)(image_height - (BOXSIZE - 1) / 2 - 1);
            ...
            for (c = cstart, i = 0; c < cfinish; c++, i++)
            {
                ...
                {
                    gx = (image[(r * image_width) + c + 1] -
                          image[(r * image_width) + c - 1]) / 2.0f;
                    gy = (image[((r + 1) * image_width) + c] -
                          image[((r - 1) * image_width) + c]) / 2.0f;
            ...
            for (v = vstart; v < vfinish; v++)
            {
                ...
                if (v != vfinish - 1)
                {
                    ...
                    for (c = cstart, i = 0; c < cfinish; c++, i++)
                    {
                        ...
                        gx = (image[((v - (BOXSIZE - 1) / 2) * image_width) + c + 1] -
                              image[((v - (BOXSIZE - 1) / 2) * image_width) + c - 1]) / 2.0f;
                        gy = (image[((v - (BOXSIZE - 1) / 2 + 1) * image_width) + c] -
                              image[((v - (BOXSIZE - 1) / 2 - 1) * image_width) + c]) / 2.0f;
                        CSgxsq[i] -= gx * gx;
                        CSgysq[i] -= gy * gy;
                        CSgxgy[i] -= gx * gy;

                        // Add the new bottom pixel
                        gx = (image[((v + (BOXSIZE - 1) / 2 + 1) * image_width) + c + 1] -
                              image[((v + (BOXSIZE - 1) / 2 + 1) * image_width) + c - 1]) / 2.0f;
                        gy = (image[((v + (BOXSIZE - 1) / 2 + 1 + 1) * image_width) + c] -
                              image[((v + (BOXSIZE - 1) / 2 + 1 - 1) * image_width) + c]) / 2.0f;
                        CSgxsq[i] += gx * gx;
                        CSgysq[i] += gy * gy;
                        CSgxgy[i] += gx * gy;
                    }
                          ...
                          uint ufinish, uint vfinish)
        {
            ...
            for (c = cstart, i = 0; c < cfinish; c++, i++)
            {
                ...
                for (r = rstart; r < rstart + BOXSIZE; r++)
                {
                    gx = (image[(r * image_width) + c + 1] - image[(r * image_width) + c - 1]) / 2.0f;
                    gy = (image[((r + 1) * image_width) + c] - image[((r - 1) * image_width) + c]) / 2.0f;
        ...
        /// <param name="patch_width">width of the patch</param>
        /// <param name="patch_height">height of the patch</param>
        ...
        /// <param name="outputimage">image to be displayed</param>
        /// <param name="outputimage_width">width of the image to be displayed</param>
        /// <param name="outputimage_height">height of the image to be displayed</param>
        /// <returns>true if the a good match is found (above CORRTHRESH2), false otherwise</returns>
        public static bool elliptical_search(
                          byte[] image, int image_width, int image_height,
                          byte[] patch, int patch_width, int patch_height,
                          Vector centre,                    //1 dimensional x,y coords
                          MatrixFixed PuInv,                //2x2 matrix
                          ref uint u, ref uint v,
                          Vector vz,
                          uint uBOXSIZE,
                          byte[] outputimage, int outputimage_width, int outputimage_height,
                          bool show_ellipses,
                          bool calibrating,
                          Random rnd)
        {
            ...
            if (ucentre + urelfinish + vz_x - (BOXSIZE - 1) / 2 > (int)(image_width) - BOXSIZE)
                urelfinish = image_width - BOXSIZE - ucentre - vz_x + (BOXSIZE - 1) / 2;
            if (vcentre + vrelstart + vz_y - (BOXSIZE - 1) / 2 < 0)
                vrelstart = (BOXSIZE - 1) / 2 - vcentre - vz_y;
            if (vcentre + vrelfinish + vz_y - (BOXSIZE - 1) / 2 > (int)(image_height) - BOXSIZE)
                vrelfinish = (int)(image_height) - BOXSIZE - vcentre - vz_y + (BOXSIZE - 1) / 2;
                            ...
                            + v3 * vrelsq < NO_SIGMA * NO_SIGMA)
                        {
                            if ((show_ellipses) || (calibrating))
                            {
                                if (!inside_ellipse)
                                {
                                    ...
                                    xx2 = xx * outputimage_width / image_width;
                                    yy2 = yy * outputimage_height / image_height;
                                    outputimage[(yy2 * outputimage_width) + xx2] = (byte)255;
                                }
                                inside_ellipse = true;
                            }
